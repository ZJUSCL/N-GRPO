...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def weighted_forward(
        ...
        """Single-GPU weighted embedding forward for latent rollout."""
        assert (
            topk_probs.shape == topk_indices.shape
        ), "topk_probs and topk_indices must have same shape."
        if self.tp_size > 1:
            raise RuntimeError("weighted_forward does not support tp_size > 1")
        topk_embeddings = self.quant_method.embedding(self, topk_indices.long())
