...
class SamplingBatchInfo:
    ...
    # ==========
    latent_rollout_enable: bool = False
    hrpo_enable: bool = False
    latent_mix_strategy: Optional[str] = None
    latent_mix_top_k: int = 0
    latent_mix_temperature: float = 1.0
    latent_need_entropy: bool = False
    latent_is_validate: bool = False
    latent_neighbor_metric: Optional[str] = None
    latent_noise_rms_scale: float = 0.33
    latent_noise_on_eval: bool = False
    # ==========
    ...
    def from_schedule_batch(cls, batch: ScheduleBatch, vocab_size: int):
        ...
        # begin of latent rollout (verl)
        # ==========
        latent_cfgs = [getattr(r, "latent_cfg", None) or {} for r in reqs]
        latent_rollout_enable = any(
            cfg.get("latent_rollout_enable", False) for cfg in latent_cfgs
        )
        hrpo_enable = any(cfg.get("hrpo_enable", False) for cfg in latent_cfgs)
        latent_mix_top_k = max(int(cfg.get("latent_rollout_mix_top_k") or 0) for cfg in latent_cfgs)
        latent_mix_temperature = max(
            float(cfg.get("latent_rollout_mix_temperature", 1.0)) for cfg in latent_cfgs
        )
        latent_noise_rms_scale = max(
            float(cfg.get("latent_rollout_noise_rms_scale", 0.33)) for cfg in latent_cfgs
        )
        latent_noise_on_eval = any(
            bool(cfg.get("latent_rollout_noise_on_eval", False)) for cfg in latent_cfgs
        )
        latent_need_entropy = any(
            cfg.get("latent_rollout_mix_entropy_threshold", None) is not None
            for cfg in latent_cfgs
        )
        latent_is_validate = any(bool(cfg.get("latent_is_validate", False)) for cfg in latent_cfgs)
        strategies = {
            cfg.get("latent_rollout_mix_strategy", None)
            for cfg in latent_cfgs
            if cfg.get("latent_rollout_enable", False)
        }
        latent_mix_strategy = strategies.pop() if len(strategies) == 1 else None
        metrics = {
            cfg.get("latent_rollout_neighbor_metric", None)
            for cfg in latent_cfgs
            if cfg.get("latent_rollout_enable", False)
        }
        metrics = {m for m in metrics if m is not None}
        if len(metrics) == 1:
            latent_neighbor_metric = metrics.pop()
        elif len(metrics) == 0:
            latent_neighbor_metric = None
        else:
            latent_neighbor_metric = "__mixed__"
        # ==========
        # end of latent rollout (verl)
        # ==========
        ...
        ret = cls(
            ...
            latent_rollout_enable=latent_rollout_enable,
            hrpo_enable=hrpo_enable,
            latent_mix_strategy=latent_mix_strategy,
            latent_mix_top_k=latent_mix_top_k,
            latent_mix_temperature=latent_mix_temperature,
            latent_need_entropy=latent_need_entropy,
            latent_is_validate=latent_is_validate,
            latent_neighbor_metric=latent_neighbor_metric,
            latent_noise_rms_scale=latent_noise_rms_scale,
            latent_noise_on_eval=latent_noise_on_eval,
            vocab_size=vocab_size,
