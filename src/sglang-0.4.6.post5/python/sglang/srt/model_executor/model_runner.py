...
class ModelRunner:
    ...
    def load_model(self):
        ...
        # Enable latent rollout embedding override
        try:
            from sglang.srt.models.latent_rollout_patch import (
                apply_latent_rollout_patch,
            )

            apply_latent_rollout_patch(self.model)
        except Exception as e:  # pragma: no cover - best-effort
            logger.warning("Failed to apply latent rollout patch: %s", e)
        # Enable HRPO gating patch (best-effort)
        try:
            from sglang.srt.models.hrpo_patch import apply_hrpo_patch

            apply_hrpo_patch(self.model)
        except Exception as e:  # pragma: no cover - best-effort
            logger.warning("Failed to apply HRPO patch: %s", e)
    ...
    def forward_decode(
        ...
        self._maybe_mix_latent_embeds(forward_batch, kwargs)
    ...
    def _maybe_mix_latent_embeds(
        self, forward_batch: ForwardBatch, kwargs: dict
    ) -> None:
        """In-place mix latent embeds into input_embeds when thinking_mask is provided."""
        if getattr(getattr(forward_batch, "sampling_info", None), "hrpo_enable", False):
            return
        if (
