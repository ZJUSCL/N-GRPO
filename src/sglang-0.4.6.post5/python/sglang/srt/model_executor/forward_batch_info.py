    ...
    def init_new(
        ...
        ret = cls(
            ...
            thinking_mask=(
                ...
                if batch.thinking_mask is not None
                else None
            ),
            latent_topk_probs=(
                batch.latent_topk_probs.to(device, non_blocking=True)
                if batch.latent_topk_probs is not None
                else None
            ),
            latent_topk_indices=(
                batch.latent_topk_indices.to(device, non_blocking=True)
                if batch.latent_topk_indices is not None
                else None
            ),
            reqs=batch.reqs,
