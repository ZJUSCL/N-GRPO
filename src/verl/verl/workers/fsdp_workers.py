...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            setattr(actor_module, "hrpo_tokenizer", self.tokenizer)
            setattr(actor_module, "latent_rollout_tokenizer", self.tokenizer)
            ...
            apply_hrpo_patch(actor_module, hrpo_cfg=getattr(algo_cfg, "hrpo", None))
            apply_latent_rollout_patch(actor_module, latent_cfg=getattr(algo_cfg, "latent_rollout", None))
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        # Support all hardwares
        hrpo_cfg = getattr(getattr(self.config, "algorithm", None), "hrpo", None)
        hrpo_enabled = bool(getattr(hrpo_cfg, "enable", False))
        if hrpo_enabled and data.batch is not None:
            keep_keys = [k for k in data.batch.keys() if k not in {"thinking_embeds", "thinking_mask", "embeds_ratio"}]
            data = data.select(batch_keys=keep_keys)
