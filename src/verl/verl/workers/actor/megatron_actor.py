...
class MegatronPPOActor(BasePPOActor):
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if use_entropy_top_mask and not forward_only:
                if entropy is None:
                ...
                if entropy_mask is not None:
                    loss_mask = entropy_mask.to(response_mask.dtype)
                    kept_tokens = loss_mask.sum().to(torch.float32)
                    total_tokens = torch.clamp(response_mask.sum().to(torch.float32), min=1.0)
                    metrics["actor/entropy_mask_ratio"] = (kept_tokens / total_tokens).item()
                    if entropy_threshold is not None:
                        metrics["actor/entropy_mask_threshold"] = entropy_threshold
            ...
            if forward_only:
                ...
                if self.config.use_kl_loss:
                    ...
                    kl_loss = agg_loss(loss_mat=kld, loss_mask=loss_mask, loss_agg_mode=self.config.loss_agg_mode)
                ...
                if ratio_reg_coef:
                    embeds_ratio = data.get("embeds_ratio")
                    if embeds_ratio is not None:
                        embeds_ratio = embeds_ratio.to(policy_loss.dtype)
                        thinking_mask = data.get("thinking_mask")
                        if thinking_mask is not None:
                            ratio_values = embeds_ratio.masked_select(thinking_mask.to(torch.bool))
                        else:
                        ...
                        if ratio_values.numel() > 0:
                            ...
                            if ratio_values.numel() > 0:
                                ratio_reg = ratio_values.pow(2).mean()
                                policy_loss = policy_loss + ratio_reg_coef * ratio_reg
                                metrics["actor/latent_ratio_reg"] = ratio_reg.detach().item()
                                metrics["actor/latent_ratio_reg_coef"] = float(ratio_reg_coef)
    ...
    def update_policy(self, dataloader: Iterable[DataProto]) -> dict:
        ...
        for data in dataloader:
            ...
            calculate_entropy = self.config.entropy_coeff != 0 or use_entropy_top_mask
