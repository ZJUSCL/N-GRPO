...
def _set_envs_and_config(server_args: ServerArgs):
    ...
    if server_args.attention_backend == "flashinfer":
        assert_pkg_version(
            ...
            "0.2.5",
            (
                "Please uninstall the old version and reinstall the latest version by following the "
                "instructions at https://docs.flashinfer.ai/installation.html."
            ),
        )
    if is_cuda():
        assert_pkg_version(
...
def _post_process_outputs(processing_class, output):
    ...
    batched_top_logprobs = []
    batched_thinking_embeds = []
    batched_thinking_mask = []
    batched_embeds_ratio = []
    batched_latent_entropy = []
    batched_latent_mix_explore_mask = []
    batched_latent_topk_probs = []
    batched_latent_topk_indices = []
    for resp, (output_token_ids, log_probs) in zip(output, out_map, strict=True):
        batched_output_token_ids.append(output_token_ids)
        batched_logprobs.append(log_probs)
        meta = resp.get("meta_info", {}) if isinstance(resp, dict) else {}
        batched_hidden_states.append(meta.get("hidden_states"))
        batched_top_logprobs.append(meta.get("output_top_logprobs"))
        batched_thinking_embeds.append(meta.get("latent_thinking_embeds"))
        batched_thinking_mask.append(meta.get("latent_thinking_mask"))
        batched_embeds_ratio.append(meta.get("latent_embeds_ratio"))
        batched_latent_entropy.append(meta.get("latent_entropy"))
        batched_latent_mix_explore_mask.append(meta.get("latent_mix_explore_mask"))
        batched_latent_topk_probs.append(meta.get("latent_topk_probs"))
        batched_latent_topk_indices.append(meta.get("latent_topk_indices"))
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        self.hidden_size = self._infer_hidden_size(model_hf_config)
    ...
    @staticmethod
    def _infer_hidden_size(model_hf_config):
        hidden_size = getattr(model_hf_config, "hidden_size", None)
        if hidden_size is not None:
            return hidden_size
        for attr in ("llm_config", "text_config"):
            inner = getattr(model_hf_config, attr, None)
            if inner is not None and getattr(inner, "hidden_size", None) is not None:
                return inner.hidden_size
        return None
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            # allow overriding attention backend via rollout.engine_kwargs.sglang.attention_backend
            sglang_kwargs = {}
            if hasattr(self.config, "engine_kwargs") and getattr(self.config.engine_kwargs, "sglang", None):
                sglang_kwargs = dict(self.config.engine_kwargs.sglang)
            attn_backend = sglang_kwargs.get("attention_backend", None)
            mm_attn_backend = sglang_kwargs.get(
                "mm_attention_backend", attn_backend
            )
            ...
            self._engine = AsyncEngine(
                ...
                mm_attention_backend=mm_attn_backend or attn_backend or "fa3",
                attention_backend=attn_backend or "fa3",
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        is_validate = meta.get("validate", False)
        disable_eval_mix = getattr(self.config, "latent_rollout_disable_mix_on_eval", False)
        latent_enabled = getattr(self.config, "latent_rollout_enable", False)

        # eval 时若显式关闭混合，直接走普通离散生成路径
        if latent_enabled and not (disable_eval_mix and is_validate):
            if getattr(self.config, "hrpo_enable", False):
                raise ValueError("latent_rollout_enable 和 hrpo_enable 不能同时开启")
            return self._batch_level_generate_sequences_latent(prompts, **kwargs)
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if getattr(self.config, "hrpo_enable", False):
            ...
            answer_start = getattr(self.config, "hrpo_answer_start", "####")
            answer_token_ids = tokenizer(answer_start, add_special_tokens=False).input_ids
            if len(answer_token_ids) == 0:
                raise ValueError(
                    f"无法从 tokenizer 中得到有效的 HRPO answer_start 编码：{answer_start}"
                )
            request_sampling_params["custom_params"] = [
                {
                    "hrpo_enable": True,
                    "hrpo_answer_start_ids": answer_token_ids,
                }
                for _ in idx_list
            ]
    ...
    @GPUMemoryLogger(role="sglang rollout (latent)", logger=logger)
    @torch.no_grad()
    def _batch_level_generate_sequences_latent(self, prompts: DataProto, **kwargs) -> DataProto:
        """单轮 latent GRPO 生成。支持 mix_top_k / gumbel 等与 HF 对齐的混合。"""
        ...
        answer_start = getattr(self.config, "latent_rollout_answer_start", "####")
        answer_token_ids = tokenizer(answer_start, add_special_tokens=False).input_ids
        if len(answer_token_ids) == 0:
            raise ValueError(
                f"无法从 tokenizer 中得到有效的 answer_start 编码：{answer_start}"
            )
        answer_token_tensor = torch.tensor(answer_token_ids, device=prompts.batch["input_ids"].device)
        answer_len = answer_token_tensor.size(0)
        ...
        if mix_top_k is None or int(mix_top_k) <= 0:
        ...
        # 按 batch 准备 latent 配置
        latent_enable_flag = not (
            getattr(self.config, "latent_rollout_disable_mix_on_eval", False) and is_validate
        )
        latent_custom_params = []
        for _ in range(batch_size):
            latent_custom_params.append(
                {
                    "latent_rollout_enable": latent_enable_flag,
                    "latent_rollout_mix_top_k": getattr(self.config, "latent_rollout_mix_top_k", None),
                    "latent_rollout_mix_temperature": getattr(self.config, "latent_rollout_mix_temperature", 1.0),
                    "latent_rollout_mix_entropy_threshold": getattr(
                        self.config, "latent_rollout_mix_entropy_threshold", None
                    ),
                    "latent_rollout_mix_strategy": getattr(self.config, "latent_rollout_mix_strategy", None),
                    "latent_rollout_neighbor_metric": getattr(self.config, "latent_rollout_neighbor_metric", "cosine"),
                    "latent_rollout_mix_rate": getattr(self.config, "latent_rollout_mix_rate", 1.0),
                    "latent_rollout_disable_mix_on_eval": getattr(self.config, "latent_rollout_disable_mix_on_eval", False),
                    "latent_rollout_noise_rms_scale": getattr(self.config, "latent_rollout_noise_rms_scale", 0.33),
                    "latent_rollout_noise_on_eval": getattr(self.config, "latent_rollout_noise_on_eval", False),
                    "latent_rollout_force_decode_interval": getattr(
                        self.config, "latent_rollout_force_decode_interval", None
                    ),
                    "latent_rollout_force_decode_burn_in": getattr(
                        self.config, "latent_rollout_force_decode_burn_in", 0
                    ),
                    "latent_rollout_answer_start_ids": answer_token_ids,
                    "latent_is_validate": is_validate,
                }
            )
        request_sampling_params["custom_params"] = latent_custom_params
        ...
        if self._tp_rank == 0:
            output = self._engine.generate(
                ...
                input_ids=idx_list,
                image_data=image_list,
                # 不再请求 full hidden_states，减少跨进程拷贝与显存占用
        ...
        (
            ...
            _hidden_states_list,
            _,
            thinking_embeds_list,
            thinking_mask_list,
            embeds_ratio_list,
            latent_entropy_list,
            latent_mix_explore_mask_list,
            latent_topk_probs_list,
            latent_topk_indices_list,
        ) = _post_process_outputs(
            self.processing_class, output
        )

        response = out_token_ids.to(idx.device)
        rollout_log_probs = out_log_probs.to(idx.device)
        ...
        lightweight_gaussian = mix_strategy == "gaussian_embed_noise"

        if not lightweight_gaussian:
            hidden_size = self.hidden_size
            if hidden_size is None:
                raise RuntimeError(
                    "模型配置未提供 hidden_size；在禁用 hidden_states 返回后，需要在模型 config 或加载参数中显式声明 hidden_size。"
                )
            ...
            thinking_mask = torch.zeros((batch_size, total_len), device=idx.device, dtype=torch.bool)
            embeds_ratio = torch.zeros_like(thinking_mask, dtype=torch.float32)

        latent_entropy = torch.zeros((batch_size, total_len), device=idx.device, dtype=torch.float32)
        latent_mix_explore_mask = torch.zeros((batch_size, total_len), device=idx.device, dtype=torch.bool)

        for i in range(batch_size):
            embed_list = thinking_embeds_list[i] or []
            mask_list = thinking_mask_list[i] or []
            ratio_list = embeds_ratio_list[i] or []
            entropy_list = latent_entropy_list[i] or []
            mix_mask_list = latent_mix_explore_mask_list[i] or []

            resp_start = prompt_len
            if lightweight_gaussian:
                max_len = max(len(entropy_list), len(mix_mask_list))
            else:
                max_len = max(
                    len(embed_list),
                    len(mask_list),
            ...
            for pos in range(min(response_length, max_len)):
                if pos < len(entropy_list):
                    latent_entropy[i, resp_start + pos] = float(entropy_list[pos])
                if pos < len(mix_mask_list):
                    latent_mix_explore_mask[i, resp_start + pos] = bool(mix_mask_list[pos])
                if not lightweight_gaussian:
                    if pos < len(embed_list):
                        thinking_embeds[i, resp_start + pos, : len(embed_list[pos])] = torch.tensor(
                            embed_list[pos], device=idx.device, dtype=torch.float32
                        )
                    if pos < len(mask_list):
                        thinking_mask[i, resp_start + pos] = bool(mask_list[pos])
                    if pos < len(ratio_list):
                        embeds_ratio[i, resp_start + pos] = float(ratio_list[pos])

        if mix_strategy == "orthogonal_noise":
            for i in range(batch_size):
                embed_list = thinking_embeds_list[i] or []
                generated_len = int((response[i] != self.pad_token_id).sum().item())
                if len(embed_list) < generated_len:
                    raise RuntimeError(
                        f"latent embed list 缺失：batch={i} generated_len={generated_len} "
                        f"embeds_len={len(embed_list)}"
                    )

        if mix_strategy != "orthogonal_noise":
            # top-k info for latent rollout (prompt uses one-hot, response uses rollout top-k)
            max_topk = 1
            for probs_list in latent_topk_probs_list:
                if probs_list:
                    for probs in probs_list:
                        if probs:
                            max_topk = max(max_topk, len(probs))
            latent_topk_indices = torch.full(
                (batch_size, total_len, max_topk),
                fill_value=self.pad_token_id,
                device=idx.device,
                dtype=torch.long,
            )
            latent_topk_probs = torch.zeros(
                (batch_size, total_len, max_topk),
                device=idx.device,
                dtype=torch.float32,
            )
            latent_topk_indices[:, :prompt_len, 0] = idx
            latent_topk_probs[:, :prompt_len, 0] = 1.0

            for i in range(batch_size):
                resp_start = prompt_len
                resp_indices_list = latent_topk_indices_list[i] or []
                resp_probs_list = latent_topk_probs_list[i] or []
                response_mask = get_response_mask(
                    response_id=response[i].unsqueeze(0),
                    eos_token=eos_token_id,
                    dtype=torch.bool,
                ).squeeze(0)
                generated_len = int(response_mask.sum().item())
                for pos in range(response_length):
                    if pos < len(resp_indices_list) and resp_indices_list[pos]:
                        inds = resp_indices_list[pos]
                        probs = resp_probs_list[pos] if pos < len(resp_probs_list) else None
                        k = min(len(inds), max_topk)
                        latent_topk_indices[i, resp_start + pos, :k] = torch.tensor(
                            inds[:k], device=idx.device, dtype=torch.long
                        )
                        if probs:
                            latent_topk_probs[i, resp_start + pos, :k] = torch.tensor(
                                probs[:k], device=idx.device, dtype=torch.float32
                            )
                        else:
                            latent_topk_probs[i, resp_start + pos, 0] = 1.0
                    else:
                        if pos < generated_len:
                            print(
                                f"[latent_mix warn] missing top-k, fallback one-hot: batch={i} pos={pos}"
                            )
                        latent_topk_indices[i, resp_start + pos, 0] = response[i, pos]
                        latent_topk_probs[i, resp_start + pos, 0] = 1.0
        ...
        if not lightweight_gaussian:
            batch_tensors["thinking_embeds"] = thinking_embeds
            batch_tensors["thinking_mask"] = thinking_mask
            batch_tensors["embeds_ratio"] = embeds_ratio

        batch = TensorDict(batch_tensors, batch_size=batch_size)
        if mix_strategy != "orthogonal_noise":
            batch["latent_topk_probs"] = latent_topk_probs
            batch["latent_topk_indices"] = latent_topk_indices
        if self.config.calculate_log_probs:
            batch["rollout_log_probs"] = rollout_log_probs
