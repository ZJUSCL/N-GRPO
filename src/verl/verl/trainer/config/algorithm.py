...
@dataclass
class HRPOConfig(BaseConfig):
    """Configuration for hybrid reasoning policy optimisation."""

    _frozen_fields = ["enable"]
...
@dataclass
class LatentRolloutConfig(BaseConfig):
    """Configuration for latent-state rollouts with explicit stochasticity."""

    _frozen_fields = ["enable"]

    enable: bool = False
    answer_start: str = "####"
    force_decode_interval: Optional[int] = None
    force_decode_burn_in: int = 0
    mix_top_k: Optional[int] = None
    mix_temperature: float = 1.0
    mix_entropy_threshold: Optional[float] = None
    mix_rate: Optional[float] = None
    # topk/topk-gumbel/neighbor/orthogonal_noise/gaussian_embed_noise
    mix_strategy: Optional[str] = None
    neighbor_metric: str = "cosine"  # cosine/dot/l1/l2 (only used when mix_strategy == "neighbor")
    disable_mix_on_eval: bool = False
    noise_rms_scale: float = 0.33
    noise_on_eval: bool = False
...
class AlgoConfig(BaseConfig):
    ...
    hrpo: Optional[HRPOConfig] = None
    latent_rollout: Optional[LatentRolloutConfig] = None
