...
@register_adv_est(AdvantageEstimator.GSPO)  # or simply: @register_adv_est("gspo")
def compute_gspo_outcome_advantage(
    ...
    **kwargs,
    ...
    """
    Compute advantage for GSPO, using the same group-relative outcome reward
    formulation as GRPO. GSPO differs in the policy loss (sequence-level ratio).
    """
...
def compute_policy_loss_gspo(
    ...
    """
    Compute sequence-level clipped policy objective for GSPO.

    GSPO uses a sequence-level importance ratio (optionally length-normalized)
    and applies PPO-style clipping at the sequence level.
    """
    if config is None:
        raise ValueError("GSPO policy loss requires config to access clip ratios.")
    ...
    seq_lens = response_mask.sum(dim=-1)
    seq_mask = seq_lens > 0
    seq_lens_clamped = torch.clamp(seq_lens, min=1.0)

    log_ratio = (log_prob - old_log_prob) * response_mask
    log_ratio = log_ratio.sum(dim=-1)
    if config.policy_loss.get("gspo_length_norm", True):
        log_ratio = log_ratio / seq_lens_clamped
    log_ratio = torch.clamp(log_ratio, min=-20.0, max=20.0)
    ratio = torch.exp(log_ratio)

    seq_adv = (advantages * response_mask).sum(dim=-1) / seq_lens_clamped

    pg_losses1 = -seq_adv * ratio
    pg_losses2 = -seq_adv * torch.clamp(ratio, 1 - cliprange_low, 1 + cliprange_high)
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
    pg_clipfrac = (pg_losses2 > pg_losses1).float()

    pg_losses3 = -seq_adv * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = torch.gt(clip_pg_losses1, pg_losses3) * (seq_adv < 0).float()

    pg_losses = torch.where(seq_adv < 0, clip_pg_losses2, clip_pg_losses1)

    denom = torch.clamp(seq_mask.float().sum(), min=1.0)
    pg_loss = (pg_losses * seq_mask.float()).sum() / denom
    pg_clipfrac = (pg_clipfrac * seq_mask.float()).sum() / denom
    pg_clipfrac_lower = (pg_clipfrac_lower * seq_mask.float()).sum() / denom
    ppo_kl = (-log_ratio * seq_mask.float()).sum() / denom
