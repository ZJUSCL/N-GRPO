...
class TestComputeDataMetrics(unittest.TestCase):
    ...
    def test_compute_data_metrics_with_critic(self):
        ...
        self.assertIn("critic/score/std", metrics)
        self.assertIn("critic/rewards/mean", metrics)
        self.assertIn("critic/rewards/std", metrics)
        ...
        self.assertIn("critic/returns/std", metrics)
        self.assertIn("critic/values/mean", metrics)
        self.assertIn("critic/values/std", metrics)
        self.assertIn("critic/vf_explained_var", metrics)
        self.assertIn("response_length/mean", metrics)
        self.assertIn("response_length/std", metrics)
        self.assertIn("prompt_length/mean", metrics)
        self.assertIn("prompt_length/std", metrics)

        # Check some specific values
        self.assertAlmostEqual(metrics["critic/score/mean"], 5.0)  # Sum of token_level_scores
        self.assertAlmostEqual(metrics["critic/score/std"], 2.0)
        self.assertAlmostEqual(metrics["critic/rewards/mean"], 2.5)  # Sum of token_level_rewards
        self.assertAlmostEqual(metrics["critic/rewards/std"], 1.0)
        self.assertAlmostEqual(metrics["critic/advantages/std"], np.std([0.1, 0.2, 0.3, 0.4]))
        self.assertAlmostEqual(metrics["critic/returns/std"], np.std([1.1, 1.2, 1.3, 1.4]))
        self.assertAlmostEqual(metrics["critic/values/std"], np.std([0.9, 1.0, 1.1, 1.2]))
        self.assertAlmostEqual(metrics["response_length/std"], 0.0)
        self.assertAlmostEqual(metrics["prompt_length/std"], 0.0)
    ...
    def test_compute_data_metrics_without_critic(self):
        ...
        self.assertNotIn("critic/values/std", metrics)
